@php
    ...
    $orderStatu = array_filter($orderStatuses, function ($status) {
            ...
            <div class="row g-3">
                <div class="col-md-6 col-lg-4 col-xl-3">
                    <div class="dashboard-box item-1">
                        ...
                        <h3 class="title {{ app()->getLocale() == 'ar' ? 'margin-51' : '' }}">{{ __($text) }}</h3>
                        <div class="icon">
                ...
                <div class="col-md-6 col-lg-4 col-xl-3">
                    <div class="dashboard-box item-2">
                        ...
                        <h3 class="title {{ app()->getLocale() == 'ar' ? 'margin-51' : '' }}">{{ __('Total Products') }}</h3>
                        <div class="icon">
                ...
                <div class="col-md-6 col-lg-4 col-xl-3">
                    <div class="dashboard-box item-3">
                        ...
                        <h3 class="title {{ app()->getLocale() == 'ar' ? 'margin-51' : '' }}">{{ __('Total Orders') }}</h3>
                        <div class="icon">
                ...
                <div class="col-md-6 col-lg-4 col-xl-3">
                    <div class="dashboard-box item-4">
                        ...
                        <h3 class="title {{ app()->getLocale() == 'ar' ? 'margin-51' : '' }}">{{ __('Total Customers') }}</h3>
                        <div class="icon">
                ...
                <div class="d-flex flex-wrap gap-3 orderStatus">
                    @foreach ($orderStatuses2 as $status)
                        <a href="{{ route('admin.order.index', [str_replace(' ', '_', $status->value) , 1]) }}"
                            ...
                            <span class="count">{{ ${Str::camel($status->value . '1')} }}</span>
                        </a>
                    @endforeach
...
<div id="product-sales-overview" class="container my-4">
    ...
    <div class="row mb-4">
        ...
        <div class="col-xxl-4 col-lg-6 mt-3">
            <div class="card">
                <div class="card-body">
                    <div class="cardTitleBox">
                        <h5 class="card-title chartTitle">
                            {{ __('Less Selling Products') }}
                        </h5>
                    </div>

                    <div class="d-flex flex-column gap-1">
                        @foreach ($lseeSellingProducts as $product)
                            <a href="{{ route('admin.product.show', $product->id) }}" class="customer-section">
                                <div class="customer-details">
                                    <div class="customer-image">
                                        <img src="{{ $product->thumbnail }}" alt="icon" loading="lazy" />
        ...
        <!-- Top Customers Selling Products -->
        <div class="col-xxl-4 col-lg-6 mt-3">
            <div class="card">
                <div class="card-body">
                    <div class="cardTitleBox">
                    ...
                    <div class="d-flex flex-column gap-1">
                        @foreach ($topCustomers as $topCustomer)
                                <div class="customer-details d-flex justify-content-between align-items-center">
                                    <div class="customer-image d-flex g-3">
                                        <img src="{{ $topCustomer->user->media ?? 'default/profile.jpg' }}" alt="icon" loading="lazy" />
                                        <div class="customer-about d-flex align-items-center gap-3">
                                            <p class="text-dark name">
                                                {{ Str::limit($topCustomer->user->name, 30, '...') }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="border text-black px-2 py-1 flex-shrink-0" style="font-size: 13px; border-radius: 25px;">
                                        <div>{{ __('Purchases Count') }}: {{ $topCustomer->orders_count }}</div>
                                    </div>
                                </div>
                        @endforeach
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        var themeColor = "{{ $generaleSetting?->primary_color ?? '#689c41' }}";
        var themeHoverColor = "{{ $generaleSetting?->secondary_color ?? '#e1ebd9' }}";
        ...
        const labelsData = ismultiShop ? ["{{ __('Customer') }}", "{{ __('Shop') }}", "{{ __('Shipping companies') }}"] : ["{{ __('Customer') }}", "{{ __('Shipping companies') }}"];
