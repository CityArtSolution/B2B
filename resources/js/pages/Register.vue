        ...
        <div class="card-body"
          ...
          <form @submit.prevent="registerFormSubmit()">
            <div class="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label for="first_name" class="block mb-1">
                  <template v-if="master.langDirection === 'rtl'">الاسم الأول</template>
                  <template v-else>{{ $t('First Name') }}</template>
                </label>
                <input
                    type="text"
                    id="first_name"
                    v-model="registerFormData.first_name"
                    :placeholder="master.langDirection === 'rtl' ? 'الاسم الأول' : $t('Enter first name')"
                    class="form-control"
                />
                <span v-if="errors?.first_name" class="text-sm text-red-500 mt-1 block">
        {{ errors.first_name[0] }}
      </span>
              </div>
              ...
              <div>
                <label for="last_name" class="block mb-1">
                  <template v-if="master.langDirection === 'rtl'">الاسم الأخير</template>
                  <template v-else>{{ $t('Last Name') }}</template>
                </label>
                <input
                    type="text"
                    id="last_name"
                    v-model="registerFormData.last_name"
                    :placeholder="master.langDirection === 'rtl' ? 'الاسم الأخير' : $t('Enter last name')"
                    class="form-control"
                />
                <span v-if="errors?.last_name" class="text-sm text-red-500 mt-1 block">
        {{ errors.last_name[0] }}
      </span>
              </div>
            </div>
            <div class="mb-3">
              ...
              <input type="text" id="reg-phone" v-model="registerFormData.phone" class="form-control" />
              <span v-if="errors?.phone" class="text-danger">{{ errors.phone[0] }}</span>
            </div>

            <div class="mb-3 form-check d-flex" :style="{ textAlign: master.langDirection === 'rtl' ? 'right' : 'left' }">
              <input
                  type="checkbox"
                  class="form-check-input"
                  id="hasCommercial"
                  v-model="registerFormData.has_commercial_register"
              />
              <label class="form-check-label ms-2" for="hasCommercial">
                <template v-if="master.langDirection === 'rtl'">لديك سجل تجاري؟</template>
                <template v-else>{{ $t('Have Commercial Register?') }}</template>
              </label>
            </div>

            <transition name="fade">
              <div v-if="registerFormData.has_commercial_register">
                <div class="mb-3">
                  <label>
                    <template v-if="master.langDirection === 'rtl'">رقم السجل التجاري</template>
                    <template v-else>{{ $t('Commercial Register Number') }}</template>
                  </label>
                  <input type="text" v-model="registerFormData.commercial_register" class="form-control" />
                  <span v-if="errors?.commercial_register" class="text-danger">{{ errors.commercial_register[0] }}</span>
                </div>

                <div class="mb-3">
                  <label>
                    <template v-if="master.langDirection === 'rtl'">الرقم الضريبي</template>
                    <template v-else>{{ $t('Tax ID Number') }}</template>
                  </label>
                  <input type="text" v-model="registerFormData.tax_id" class="form-control" />
                  <span v-if="errors?.tax_id" class="text-danger">{{ errors.tax_id[0] }}</span>
                </div>
              </div>
            </transition>

            <button v-if="!isLoading" class="btn loginButton" type="submit">
              <template v-if="master.langDirection === 'rtl'">إنشاء حساب</template>
              <template v-else>{{ $t('Register') }}</template>
            </button>
            <button v-else disabled class="btn loginButton" style="opacity: 0.7;">
              <LoadingSpin />
            </button>
          </form>
          <div class="mt-4 text-center registration-link">
            ...
            <template v-if="master.langDirection === 'rtl'">
               ...
               لديك حساب؟
              <router-link :to="{ name: 'login' }" class="fw-bold text-primary text-decoration-none">
...
const registerFormSubmit = () => {
  ...
  const payload = {
    ...
    phone: Number(registerFormData.value.phone.replace(/\D/g, ''))
  };
